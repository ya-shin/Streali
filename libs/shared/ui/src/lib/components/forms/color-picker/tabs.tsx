...
export const Tabs = (props: TabsProps) => {
  ...
  const buttonActiveStyle =
    'text-white border-primary-400 border-solid border-b';

  return (
    <div className="flex flex-col items-center gap-4">
      <div className="flex items-center w-full px-2 gap-2">
        <div className="w-4 h-4 shrink-0 rounded overflow-hidden"style={{
              backgroundColor: `${hsvaToRgbaString(color)}`,
            }}>
        </div>

        {tab === 'hex' && (
          <TabHex color={color} onChange={(e: HsvaColor) => onChange(e)} />
        )}
        {tab === 'rgba' && (
          <TabRGB color={color} onChange={(e: HsvaColor) => onChange(e)} />
        )}
        {tab === 'hsla' && (
          <TabHSL color={color} onChange={(e: HsvaColor) => onChange(e)} />
        )}
      </div>
